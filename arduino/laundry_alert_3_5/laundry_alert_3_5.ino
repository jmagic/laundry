...
int washer = 0;    // used to store old status
int dryer = 0;     // same as washer
int alert = 0;
int old_alert = 5;  // used to store alert status alert 00 is all off ; alert 01 is washer on; alert 10 is dryer on; alert 11 all on
int washer_status = 0;  // stores actual status (probably redundant could just measure at the time)
int dryer_status = 0;   // same for dryer
int button_status = 0;  // same for button
...
void loop()
{
    ...
    if (washer == 0 and washer_status == 1)   // check the old status vs the new status.
                                              // this is due to the way the washer works.  
                                              // its light is off, you turn it on and the 
                                              // light is on, then you turn it off and 
                                              // it is time to indicate
     {
        washer = 1;
     }
     
    if (dryer == 0 and dryer_status == 1)
     {
       dryer = 1;
     }
    if (washer == 1 and washer_status == 0 and alert != 1 and alert != 11)
     {
       alert = alert + 1;
       washer = 0;
       washer_time = millis(); // our start time for the washer
     }
    if (dryer == 1 and dryer_status == 0 and alert != 10 and alert != 11)
     { 
       alert = alert + 10;
       dryer = 0;
       dryer_time = millis(); // our start time for the dryer
     }
